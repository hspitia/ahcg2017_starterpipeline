...
def main(trim_path, bowtie_path, picard_path, gatk_path, samtools_path,
         freec_path, input_path, index_path, dbsnp_path, 
    ...
    freec_path     = os.path.abspath(freec_path)  
    ...
    if not os.path.exists(freec_path):
        raise FileNotFoundError('FRECC not found at {0}'.format(freec_path))
    ...
    #
    # Variants filtering
    vcf_filtered_path = '{0}/variants.filtered.vcf'.format(out_path, os.path.splitext(os.path.basename(sam_path))[0])
    ...
    # ====================================================================
    # CNV calling using FREEC
    cnvs_path = '{0}/variants.filtered.vcf'.format(out_path, os.path.splitext(os.path.basename(sam_path))[0])
    fccmd      = [freec_path, '-config', gatk_path, '-T', 'SelectVariants', '-R', ref_path, 
                '-V', vcf_path, '-select', 'DP>=25 && QUAL>=30', 
                '-o', vcf_filtered_path]
    
    fcrun = subprocess.Popen(fccmd, shell=False)
    fcrun.wait()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--freec_conf', dest='freec_conf_path', type=str, help='Path to FREEC config file')
    ...
    parser.add_argument('-f', '--freec', dest='freec_path', type=str, help='Path to FREEC')
...
main(args.trim_path, args.bowtie_path, args.picard_path, args.gatk_path, args.samtools_path,
     args.freec_path, args.input_path, args.index_path, args.dbsnp_path, 
